...
function getBoardConfigTemplateInfo() {
    const boardConfigTemplate = new Map(
                               [
                                 ...
                                 ['tda54',{Cfile: "../ethphy_cpsw_icssg/templates/tda54/enet_board_cfg.c.xdt",
                                  Header: "../ethphy_cpsw_icssg/templates/tda54/enet_board_cfg.h.xdt"}],
                               ],
                             );
    console.log(boardConfigTemplate.get(common.getSocName()));
...
function getSocConfigTemplateInfo() {
    const socConfigTemplate = new Map(
                               [
                                 ...
                                 ['tda54',{Cfile: "/networking/enet_cpsw/templates/tda54/enet_soc_cfg.c.xdt"}],
                               ],
                             );
...
function getCpuInfo() {
	console.log(getCpuID());
	const cpuInfo = new Map(
                               [
                                  ['CSL_CORE_ID_MCU0_M55',{subsystem: "M55",
                                    clusternum: "0", core: "0"}],
                               ],
                             );
...
function getEnetCoreIdPrefix() {
    ...
    if(common.getSelfSysCfgCoreName().includes("mcu0-m55")) {
        return `TISCI_DEV_${coreInfo.subsystem}${coreInfo.clusternum}_CORE${coreInfo.core}`
    }
}
...
function getEnetCoreIntNumPrefix() {
    ...
    if(common.getSelfSysCfgCoreName().includes("m55")) {
        //return `CSLR_${coreInfo.subsystem}${coreInfo.clusternum}_CORE${coreInfo.core}INTR_`
    }
}
...
function getEnetResPartInfo() {
    const ResPartInfoMap = new Map(
                               [
                                 ['tda54',{numCores: 1, coreResInfo: [{txCh: {}, numRxCh: 1, numRxFlows: 1, numMacAddress: 4, numHwPush: 0}], isStaticTxChanAllocated: false}],
                            ],
                             );
